...
{
    const value=await pool.query(`SELECT password from admin_credentials where admin_id=?`,[id])
    const cred=value[0]
    return cred
}
export async function getFacultyCredentials(id)
{
    const value=await pool.query(`SELECT password from faculty_credentials where fac_id=?`,[id])
    const cred=value[0]
    return cred
}
export async function getStudentCredentials(id)
{
    const value=await pool.query(`SELECT password from student_credentials where stud_id=?`,[id])
    const cred=value[0]
    return cred
}
...
{
    const value=await pool.query(`SELECT courses.course_id as course_id, courses.name as name,courses.fac_id as fac_id,faculty.name as fac_name
    FROM courses
...
export async function AdminAddFaculty(facultyName,facultyAge,facultyGender,facultyDepartment,facultyPassword)
{
    await pool.query(
        `INSERT INTO faculty (name,age,gender,department)
        VALUES(?,?,?,?)`,[facultyName,facultyAge,facultyGender,facultyDepartment]
    )
...
{
    await pool.query(
        `INSERT INTO faculty_credentials (fac_id,password) VALUES (?,?)`,[facId,facPassword]  
      )
}
...
export async function AdminAddStudent(studentName,studentAge,studentGender,studentBranch)
{
    await pool.query
    (
        `INSERT INTO students (name,age,gender,branch)
        VALUES(?,?,?,?)`,[studentName,studentAge,studentGender,studentBranch]
    )
...
{
    await pool.query(
        `INSERT INTO student_credentials (stud_id,password) VALUES (?,?)`,[studId,studPassword]  
      )
}
...
{
    const [facultyProfile]=await pool.query(`SELECT * FROM faculty WHERE fac_id=?`,[fac_id])
...
{
    const value=await pool.query(`SELECT * FROM courses WHERE fac_id=?`,[fac_id])
    const facultyCourses=value[0]

    const [facultyName]=await pool.query(`SELECT faculty.name FROM faculty WHERE fac_id=?`,[fac_id])
...
{
    const value=await pool.query(`SELECT students_courses.stud_id as stud_id, students.name as name, students_courses.grade as grade
    FROM students_courses
    JOIN students ON students_courses.stud_id=students.stud_id
    WHERE students_courses.course_id=?
    ORDER BY students_courses.stud_id;`,[course_id])
    const Students=value[0]

    const [facultyName]=await pool.query(`SELECT faculty.name  FROM faculty WHERE fac_id=?`,[fac_id])

    const [courseName]=await pool.query(`SELECT courses.name FROM courses WHERE course_id=?`,[course_id])

    return { Students:Students, facultyName:facultyName[0],  courseName:courseName[0] };
}
...
export async function FacultyAddCourse(courseName,facId,courseStatus)
{
    ...
    (
        ...
        VALUES(?,?,?)`,[courseName,facId,courseStatus]
    )
...
{
    const [studentProfile]=await pool.query(`SELECT * FROM students WHERE stud_id=?`,[stud_id])
...
export async function getStudentCourses(stud_id)
{
    ...
    const value= await pool.query(` SELECT students_courses.course_id as course_id,courses.name as name,faculty.name as faculty, students_courses.grade as grade,
    ...
    ORDER BY students_courses.course_id`,[stud_id])
    const StudentCourses=value[0]

    const [StudentName]=await pool.query(`SELECT name FROM students WHERE stud_id=?`,[stud_id])
...
{
    await pool.query(`DELETE FROM students_courses 
    WHERE stud_id=? AND course_id=?`,[stud_id,course_id])
}
...
export async function getAvailableCourses(studId)
{
    const result=await pool.query(
        ...
        ORDER BY courses.course_id`,[studId]
    )

    const AvailableCourses=result[0]

    const [StudentName]=await pool.query(`SELECT name FROM students WHERE stud_id=?`,[studId])
